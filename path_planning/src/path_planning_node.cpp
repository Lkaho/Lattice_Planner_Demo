...
void PathPlanningNode::plannerTimerLoop(const ros::TimerEvent &) {
  if (!firstRecord_ && !isReachGoal_) {  //有定位数据开始规划
    ...
    std::vector<FrenetPath> candidate_traj = frenet_optimal_trajectory.GetPathList();

    std::cout << "Lattice planner worked" << std::endl;
    if (!final_path.s.empty() && !near_goal_) {
      ...
      std::cout << "planning is working!" << std::endl;
      // 可视化
      visualization_->publishLocalPlan(final_path);
      visualization_->publishAllLocalTrajectory(candidate_traj);
    ...
    if (final_path.s.empty()) {
      std::cout << "Lattice planning failed!" << std::endl;
      return;
    }
...
void PathPlanningNode::UpdateStaticObstacle() {
  std::vector<Poi_f> obstcles{{15.07, 53.53}};
  // std::vector<Poi_f> obstcles{{-100, -100}};
